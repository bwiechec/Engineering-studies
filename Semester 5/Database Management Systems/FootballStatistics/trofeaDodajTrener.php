...
<body>
    ...
    <div class="panel-body col-md-6 col-md-offset-3">
        <form role="form" action="php/dodajTrofeum.php?str=trofeaDodajTrener.php" method="POST">
            <div id="komunikat">
            <?php
            ...
            <div class="form-group">
            <label for="Trener">Wybierz zdobywcę trofeum:</label>
            <select name="Trener" id="Trener" class="form-control">
                <?php
                
                    $polaczenie = new mysqli('localhost','root','','projekt');
                    if(!$polaczenie->connect_errno){
                        $sql = "select id_trenera, Imie, Nazwisko from Trener;";
        
                        if($rezultat=$polaczenie->query($sql)){
                            
                                while($res = $rezultat->fetch_assoc()){ 
                                    echo "<option value = '$res[id_trenera]'>$res[Imie] $res[Nazwisko]</option>";
                                }
